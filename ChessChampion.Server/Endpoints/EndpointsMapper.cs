...
public static class EndpointsMapper
{
    ...
    public static async Task<Results<NoContent, BadRequest<string>, NotFound>> LeaveGame(LeaveGameRequest leaveRequest, GamesRepository gamesRepository, IHubContext<ChessHub> hub)
    {
        if (gamesRepository.TryGetGame(leaveRequest.GameId, out GameModel? game) && game is not null)
        {
            if (game.WhitePlayer?.Name == leaveRequest.UserName || game.BlackPlayer?.Name == leaveRequest.UserName)
            {
                ...
                gamesRepository.DeleteGame(leaveRequest.GameId);
                return TypedResults.NoContent();
            }
